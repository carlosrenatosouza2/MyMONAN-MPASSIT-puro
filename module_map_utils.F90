...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   TYPE proj_info
      ...
      REAL             :: lambda   ! For Rotated Lat/Lon -- domain half-extend in
                                   !  degrees longitude
      REAL(kind=8)     :: lat1     ! SW latitude (1,1) in degrees (-90->90N)
      REAL(kind=8)     :: lon1     ! SW longitude (1,1) in degrees (-180->180E)
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   SUBROUTINE ij_to_latlon(proj, i, j, lat, lon)
      ...
      REAL, INTENT(IN)                    :: i
      REAL, INTENT(IN)                    :: j
      REAL(kind=8), INTENT(OUT)                   :: lat
      REAL(kind=8), INTENT(OUT)                   :: lon
   ...
   SUBROUTINE ijll_ps(i, j, proj, lat, lon)
      ...
      ! Declare output arguments
      REAL(kind=8), INTENT(OUT)                   :: lat     ! -90 -> 90 north
      REAL(kind=8), INTENT(OUT)                   :: lon     ! -180 -> 180 East
   ...
   ! 
      ...
      ! Output Args                 
      REAL(kind=8), INTENT(OUT)             :: lat      ! Latitude (-90->90 deg N)
      REAL(kind=8), INTENT(OUT)             :: lon      ! Longitude (-180->180 E)
   ...
   ! to the cartesian x/y on a Lambert Conformal projection.
      ...
      ! Input Args
      REAL(kind=8), INTENT(IN)              :: lat      ! Latitude (-90->90 deg N)
      REAL(kind=8), INTENT(IN)              :: lon      ! Longitude (-180->180 E)
      TYPE(proj_info),INTENT(IN)      :: proj     ! Projection info structure
   ...
   SUBROUTINE set_merc(proj)
      ...
      IF (proj%lat1 .NE. 0.) THEN
         proj%rsw = (LOG(TAN(0.5*((proj%lat1+90.d0)*rad_per_deg))))/proj%dlon
      ENDIF
   ...
   SUBROUTINE llij_merc(lat, lon, proj, i, j)
      ...
      j = proj%knownj + (LOG(TAN(0.5*((lat + 90.d0) * rad_per_deg)))) / &
             proj%dlon - proj%rsw
