...
!  z_s
        ...
        !CR:inicio---
        if (.not. allocated(zs_target_grid)) allocate(zs_target_grid(nsoil_input, 1))
        zs_target_grid = missing_value
        !CR: fim---
...
!   u
       if (do_u_interp == 1) then
          ...
          if (allocated(dum3dtmp)) deallocate(dum3dtmp)
       endif
        ...
        if (interp_hist .and. n_hist_fields_soil > 0) then
            ...
            do i = 1, n_hist_fields_soil
                ...
                if (localpet == 0)  print *, trim(varname), minval(dumsoil), maxval(dumsoil)
                 ...
                 !CR:call netcdf_err(error, 'WRITING RECORD')
                 !CR:
                 if (allocated(dumsoil)) deallocate(dumsoil)
            end do
            if (allocated(fields)) deallocate (fields)
